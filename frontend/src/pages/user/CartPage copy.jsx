        ...
        <>
            ...
            <div className="max-w-7xl mx-auto py-8 px-4">
                <div className="flex items-center justify-center gap-4 mb-12">
                    <div className="flex flex-col md:flex-row items-center gap-2 text-red-900">
                        <span className="w-3 h-3 md:w-6 md:h-6 rounded-full bg-red-900 text-white flex items-center justify-center text-[9px] md:text-xs font-bold shadow-lg shadow-red-200">1</span>
                        <span className="text-[7px] md:text-xs font-bold uppercase tracking-widest">Giỏ hàng</span>
                    </div>
                    <div className="w-12 h-px bg-slate-200"></div>
                    <div className="flex flex-col md:flex-row items-center gap-2 text-slate-400">
                        <span className="w-3 h-3 md:w-6 md:h-6 rounded-full border border-slate-200 flex items-center justify-center text-[9px] md:text-xs font-bold">2</span>
                        <span className="text-[7px] md:text-xs font-bold uppercase tracking-widest">Thanh toán</span>
                    </div>
                    <div className="w-12 h-px bg-slate-200"></div>
                    <div className="flex flex-col md:flex-row items-center gap-2 text-slate-300">
                        <span className="w-3 h-3 md:w-6 md:h-6 rounded-full border border-slate-200 flex items-center justify-center text-[9px] md:text-xs font-bold">3</span>
                        <span className="text-[7px] md:text-xs font-bold uppercase tracking-widest">Hoàn tất</span>
                    </div>
                </div>
                {/* Header */}
                <div className="mb-8">
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                            ...
                            <div className="divide-y divide-gray-200">
                                {itemcart.map((cartItem) => {
                                    ...
                                    return (
                                        <div key={cartItem.uniqueId} className="p-2 md:p-6 hover:bg-gray-50 transition-colors">
                                            <div className="flex flex-row gap-4">
                                                ...
                                                >
                                                    <img
                                                        ...
                                                        alt={product.tensp}
                                                        className="w-20 h-20 md:w-30 md:h-30 sm:w-32 sm:h-32 object-cover rounded-md border border-gray-200 hover:opacity-90 transition-opacity"
                                                    />
                                                </Link>
                                                ...
                                                <div className="flex-1 flex flex-col md:flex-row md:justify-between gap-6 items-start">
                                                    ...
                                                    <div className="flex-1 space-y-4 w-full">
                                                        <div>
                                                            <Link
                                                                to={`/product/${product.slug}-${product.id}`}
                                                                className="text-xs md:text-xl font-black text-slate-800 hover:text-red-900 transition-all duration-300 leading-tight block mb-1 group"
                                                            >
                                                                {product.tensp}
                                                            </Link>
                                                            ...
                                                            {cartItem.selectedVariants && (
                                                                <div className="flex flex-wrap gap-1 mt-2">
                                                                    {cartItem.selectedVariants.color && (
                                                                        <span className="inline-flex items-center px-2 py-1 bg-slate-100 text-slate-600 text-[10px] md:text-xs font-medium rounded-md">
                                                                            {cartItem.selectedVariants.color}
                                                                        </span>
                                                                    )}
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <span className="text-md md:text-2xl font-black text-red-900 tracking-tighter">
                                                                    {formatPrice(giasp)}<span className="text-xs md:text-sm ml-0.5">đ</span>
                                                                </span>
                                                                {cartItem.variantPrice && cartItem.variantPrice !== product.giasp && (
                                                                    <span className="text-[10px] md:text-xs font-bold text-slate-400 line-through decoration-slate-300">
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <span className="hidden md:block text-[10px] font-black text-slate-400 uppercase tracking-widest">Số lượng</span>
                                                            <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-slate-50">
                                                        ...
                                                        <svg xmlns="www.w3.org" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="transition-transform"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
                                                            <span className="hidden md:block text-xs font-black uppercase tracking-widest">Loại bỏ</span>
                                                        </button>
                                                    </div>
                                                </div>
