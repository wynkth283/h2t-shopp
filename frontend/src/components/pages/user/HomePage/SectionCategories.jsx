...
export default function SectionCategories() {
    ...
    return (
        <div className="py-20 relative overflow-hidden">
            ...
            {/* Header Section */}
            <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
                <div className="space-y-2">
                    <TitleSection>Danh mục nổi bật</TitleSection>
                </div>
                <button
                    onClick={ClickToCatPage}
                    className="text-slate-400 text-xs font-bold uppercase tracking-widest hover:text-red-900 transition-colors flex items-center gap-2 group">
            ...
            <div className="relative group">
                ...
                <div className="absolute inset-y-0 left-0 w-40 bg-gradient-to-r from-[#fafafa] via-[#fafafa]/50 to-transparent z-20 pointer-events-none"></div>
                <div className="absolute inset-y-0 right-0 w-40 bg-gradient-to-l from-[#fafafa] via-[#fafafa]/50 to-transparent z-20 pointer-events-none"></div>
                    ...
                    <ContainerScrollInViewIn className="track-infinite flex gap-6">
                        {[...danhmucs, ...danhmucs].map((item, index) => (
                            <ItemScrollInViewLeft
                                ...
                                className="w-[240px] group/item cursor-pointer"
                            >
                                <div
                                onClick={() => handleToCat(bokhoantrang(item.tendanhmuc))} className="relative h-[320px] rounded-[48px] overflow-hidden shadow-xl transition-all duration-700 ease-out group-hover/item:-translate-y-4 group-hover/item:shadow-2xl group-hover/item:shadow-red-900/20">
                                    
                                    {/* 2. Ảnh Background cho từng Card */}
                                    ...
                                    <div className="relative h-full z-10 p-8 flex flex-col items-center justify-end text-center">
                                        ...
                                        {/* Icon bay lơ lửng khi hover */}
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-white/20 backdrop-blur-md border border-white/30 rounded-full flex items-center justify-center text-white opacity-0 scale-50 group-hover/item:opacity-100 group-hover/item:scale-100 transition-all duration-500">
                                        ...
                                        <div className="space-y-2 translate-y-4 group-hover/item:translate-y-0 transition-transform duration-500">
                                            ...
                                            <h3 className="font-black text-white text-xl tracking-tighter uppercase leading-tight">
                                                {item.tendanhmuc}
                                            </h3>
                                            <div className="pt-4 opacity-0 group-hover/item:opacity-100 transition-opacity duration-700">
